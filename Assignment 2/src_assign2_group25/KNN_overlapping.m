...
for i = 1:size(X_train1, 1)
    ...
    ctr = 1;
    for j = 1:size(X_train1, 1)
        closest(ctr, 1) = sum((X_train1(j, :)-X_train1(i,:)).^2);
        closest(ctr, 2) = 1;
        ctr = ctr+1;
    end
    for j = 1:size(X_train2, 1)
        closest(ctr, 1) = sum((X_train2(j, :)-X_train1(i,:)).^2);
        closest(ctr, 2) = 2;
        ctr = ctr+1;
    end
    for j = 1:size(X_train3, 1)
        closest(ctr, 1) = sum((X_train3(j, :)-X_train1(i,:)).^2);
        closest(ctr, 2) = 3;
        ctr = ctr+1;
    end
    closest = sortrows(closest, 1);
    closest = closest(1:k, :);
    conf_matrix_train(1, mode(closest(:, 2))) = conf_matrix_train(1, mode(closest(:, 2)))+1;
end
...
for i = 1:size(X_train2, 1)
    ...
    ctr = 1;
    for j = 1:size(X_train1, 1)
        closest(ctr, 1) = sum((X_train1(j, :)-X_train2(i,:)).^2);
        closest(ctr, 2) = 1;
        ctr = ctr+1;
    end
    for j = 1:size(X_train2, 1)
        closest(ctr, 1) = sum((X_train2(j, :)-X_train2(i,:)).^2);
        closest(ctr, 2) = 2;
        ctr = ctr+1;
    end
    for j = 1:size(X_train3, 1)
        closest(ctr, 1) = sum((X_train3(j, :)-X_train2(i,:)).^2);
        closest(ctr, 2) = 3;
        ctr = ctr+1;
    end
    closest = sortrows(closest, 1);
    closest = closest(1:k, :);
    conf_matrix_train(2, mode(closest(:, 2))) = conf_matrix_train(2, mode(closest(:, 2)))+1;
end
...
for i = 1:size(X_train3, 1)
    ...
    ctr = 1;
    for j = 1:size(X_train1, 1)
        closest(ctr, 1) = sum((X_train1(j, :)-X_train3(i,:)).^2);
        closest(ctr, 2) = 1;
        ctr = ctr+1;
    end
    for j = 1:size(X_train2, 1)
        closest(ctr, 1) = sum((X_train2(j, :)-X_train3(i,:)).^2);
        closest(ctr, 2) = 2;
        ctr = ctr+1;
    end
    for j = 1:size(X_train3, 1)
        closest(ctr, 1) = sum((X_train3(j, :)-X_train3(i,:)).^2);
        closest(ctr, 2) = 3;
        ctr = ctr+1;
    end
    closest = sortrows(closest, 1);
    closest = closest(1:k, :);
    conf_matrix_train(3, mode(closest(:, 2))) = conf_matrix_train(3, mode(closest(:, 2)))+1;
end
