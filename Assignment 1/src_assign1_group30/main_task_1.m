...
for i = folders
    for j = 1:10
        mat(:, ctr) = double(reshape(imread(sprintf('../data_assign1_group30/EigenAnalysis_Data/Datasets1/64x64/%d/%d.pgm', i, j)), 4096, 1));
        ctr = ctr + 1;
    end
end
...
for folder_num = randperm(15, 5)
    ...
    orig_img = double(reshape(imread(sprintf('../data_assign1_group30/EigenAnalysis_Data/Datasets1/64x64/%d/%d.pgm', folder, img_num)), 4096, 1));
    ctr = 1;
    for i = [1, 10, 20, 40, 80, 160, 320, 640]
        recon_img = mean_val;
        for j = 1:i
            recon_img = recon_img + ((orig_img-mean_val)'*V(:, end-j+1))*V(:, end-j+1);
        end
